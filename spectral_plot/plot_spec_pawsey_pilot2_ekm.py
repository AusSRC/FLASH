...
#   updated Aril 2022 for pilot 2 data
...
def make_plot(freq,chan,flux,opd,noiseflux,noiseopd,z,compno, compname, peak_flux):
    ...
    plt.savefig('outputs/%s/spectra_plots/SB%s_component_%s_flux.png'%(sbid,sbid,compno),dpi=300)
    plt.close()
    ...
    plt.savefig('outputs/%s/spectra_plots/SB%s_component_%s_opd.png'%(sbid,sbid,compno),dpi=300)
    plt.close()
...
def write_ascii(sbid,compid,chan,freq,flux,z,noiseflux,opd,noiseopd):
    print(f'In write_ascii: compid = {compid}, compno = {compno}')
    ...
    ascii.write(data_flux, 'outputs/%s/spectra_ascii/SB%s_component_%s_flux.dat'%(sbid,sbid,compno), include_names=['freq(MHz)','flux(Jy)','noise(Jy)'], format='commented_header', comment='#', delimiter=' ',overwrite=True)
    ascii.write(data_opd, 'outputs/%s/spectra_ascii/SB%s_component_%s_opd.dat'%(sbid,sbid,compno), include_names=['freq(MHz)','opd','opdnoise'], format='commented_header', comment='#', delimiter=' ',overwrite=True)
    return
...
########Start main program###########
# Working directory should be parent of 'data' directory

# 'data' directory of structure:
#
#   data --- model.txt
#       ---> sourceSpectra/
#               ---> <SBID>/SourceSpectra/ <*.fits>
#       ---> catalogues/ <component | island.xml>
#       ---> noise/
#               ---> <SBID>/NoiseSpectra/ <noise component.fits>
#
#
#
#
...
if options.sbid=='all':
    sbid_lst=glob.glob('data/sourceSpectra/3*') # - GWHG
...
for sbid in sbid_list:
    ...
    Path('outputs/%s/spectra_plots'%sbid).mkdir(parents=True,exist_ok=True)
    Path('outputs/%s/spectra_ascii'%sbid).mkdir(parents=True,exist_ok=True)
    ...
    catalogue=glob.glob('data/catalogues/selavy-image.i*.SB%s.cont.*taylor.0.restored*.components.xml'%(sbid))
...
#        opd_approx=[i/peak_flux for i in flux]
    source_list=glob.glob('data/sourceSpectra/%s/SourceSpectra/spec_*.fits'%sbid)
    for filename in source_list: 
        #compno=filename.split('_')[-1].strip('.fits') # - this will not work for all filenames as strip() also removes chars multiple times
        compno=os.path.splitext(filename.split('_')[-1])[0]
        #print(f'Opening source {filename}')
        print(f'    Processing {sbid} component {compno}, compid {compid}')
        try:
            #spechdu = fits.open('data/surceSpectra/%s/SourceSpectra/spec_SB%s_component_%s.fits'%(sbid,sbid,compno))
        ...
        contflux_freq=[]
        opd=[]
        contcube_path='data/contcubes/%s/spectra/spectrum_contcube_SB%s_component_%s.txt'%(sbid,sbid,compno)
        ...
        try:
            noisehdu = fits.open('data/noise/%s/NoiseSpectra/noise_SB%s_component_%s.fits'%(sbid,sbid,compno))
            ...
            noisepath = 'data/noise/%s/NoiseSpectra/noise_SB%s_component_%s.fits'%(sbid,sbid,noise_compno)
            if not os.path.exists(noisepath):
                ...
                print(noise_cat[idx][0],d2d.to(u.arcmin),"away")
                noise_compno=noise_cat[idx][0]
                noisehdu = fits.open('data/noise/%s/NoiseSpectra/noise_SB%s_component_%s.fits'%(sbid,sbid,noise_compno))
            else:
                print(noise_cat[idx][0],d2d.to(u.arcmin),"away")
                noise_compno=noise_cat[idx][0]
                noisehdu = fits.open('data/noise/%s/NoiseSpectra/noise_SB%s_component_%s.fits'%(sbid,sbid,noise_compno))
        ...
        plotfile='outputs/%s/spectra_plots/SB%s_component_%s_opd.png'%(sbid,sbid,compno)
