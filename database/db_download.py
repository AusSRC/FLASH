...
def set_parser():
    ...
    parser.add_argument('-m', '--mode',
            default=None,
            help='Specify run mode: PLOTS, ASCII, LINEFINDER, LINEFINDER_INVERT,LINEFINDER_MASK,LINEFINDER_INVMASK,QUERY, SQL, SOURCE (default: %(default)s)')
...
def get_files_for_sbid(conn,cur,sbid,version):
    ...
    if MODE == "ASCII":
    ...
    elif "LINEFINDER" in MODE:
        oid = None
        outputs = None
        name = f"{sbid}_{version}.tar.gz"
        mode_flags = {"LINEFINDER":False,"LINEFINDER_INVERT":False,"LINEFINDER_MASK":False,"LINEFINDER_INVMASK":False}
        mode_data = {"LINEFINDER":["detect_tar","detect_results"],
                     "LINEFINDER_INVERT":["invert_detect_tar","invert_detect_results"],
                     "LINEFINDER_MASK":["mask_detect_tar","mask_detect_results"],
                     "LINEFINDER_INVMASK":["mask_invert_detect_tar","mask_invert_detect_results"]} 

        query = "select detectionF,invert_detectionF,mask_detectionF,mask_invertF from sbid where id = %s"    
        cur.execute(query,(sid,))
        detect,invert_detect,mask_detect,invmask_detect = cur.fetchall()[0]
        modes = [detect,invert_detect,mask_detect,invmask_detect]
        mode_flags.update(zip(mode_dir.keys(), modes))
        if not mode_flags[MODE]:
            print(f"No {MODE} results available for sbid {sbid}:{version} !!")
            return
        ...
        query = f"select {mode_data[MODE][0]} from sbid where id = %s"
        #if invertF:
        ...
        if oid:
            ...
            query = f"select {mode_data[MODE][1]} from sbid where id = %s"
            #if invertF:
...
def get_results_for_sbid(cur,sbid,version,invertF,verbose=False):
    ...
    mode_files = {"LINEFINDER":"results","LINEFINDER_INVERT":"invert_results","LINEFINDER_MASK":"mask_results","LINEFINDER_INVMASK":"mask_invert_results"}
    ...
    query = f"select {mode_files[MODE]} from sbid where id = %s;"
    #if invertF:
    ...
    # if ln_mean is -1, this means get all components, even if there is no value for ln_mean
    if MODE == "LINEFINDER":
        ...
        if ln_mean != -1:
            query = query.replace("sbid_id = %s","sbid_id = %s and ln_mean > %s and mode_num > 0")
    elif MODE == "LINEFINDER_INVERT":
        ...
        if ln_mean != -1:
            query = query.replace("sbid_id = %s","sbid_id = %s and invert_ln_mean > %s and invert_mode_num > 0")
    elif MODE == "LINEFINDER_MASK":
        query = ("select component_name,comp_id,ra_hms_cont,dec_dms_cont,ra_deg_cont,dec_deg_cont,flux_peak,flux_int,has_siblings,mask_mode_num,mask_ln_mean from component where sbid_id = %s order by invert_ln_mean desc;")
        if ln_mean != -1:
            query = query.replace("sbid_id = %s","sbid_id = %s and mask_ln_mean > %s and mask_mode_num > 0")
    elif MODE == "LINEFINDER_INVMASK":
        query = ("select component_name,comp_id,ra_hms_cont,dec_dms_cont,ra_deg_cont,dec_deg_cont,flux_peak,flux_int,has_siblings,invert_mask_mode_num,invert_mask_ln_mean from component where sbid_id = %s order by invert_ln_mean desc;")
        if ln_mean != -1:
            query = query.replace("sbid_id = %s","sbid_id = %s and invert_mask_ln_mean > %s and invert_mask_mode_num > 0")
    if ln_mean == -1:
        cur.execute(query,(sid,))
    else:
    ...
    if verbose: # detailed output is saved to file
        f = open(f"{DIR}/{sbid}_{version}_{MODE}_outputs.csv","w")
...
if __name__ == "__main__":
    ...
    elif MODE in ["ASCII","LINEFINDER","LINEFINDER_INVERT","LINEFINDER_MASK","LINEFINDER_INVMASK"]:
    ...
    if MODE in ["LINEFINDER","LINEFINDER_INVERT","LINEFINDER_MASK","LINEFINDER_INVMASK"]:
