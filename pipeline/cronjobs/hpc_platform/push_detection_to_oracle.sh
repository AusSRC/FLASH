...
for SBID1 in "${SBIDARRAY[@]}"; do
    if [ "$MODE" = "STD" ]; then
        ...
        ssh -i $ORACLE_KEY flash@$CLIENT "source ~/set_local_flash_env.sh;cd ~/src/FLASH/database; python3 db_upload.py -m MASKED -s $SBID1 -t $TMPDIR -d $PARENTDIR -pw aussrc -cs config -l out_masked.log -e err_masked.log -o masked_outputs -C 'masked_linefinder_run'"
