...
for SBID1 in "${SBIDARRAY[@]}"; do
    ...
    ssh -i $ORACLE_KEY flash@$CLIENT "cd $PARENTDIR; rm -R $SBID1/inverted_outputs $1/logs $SBID1/config $TMPDIR/$SBID1*; mkdir -p $SBID1/config $SBID1/logs $SBID1/inverted_outputs;"
    # Copy data to Oracle
    scp -i $ORACLE_KEY $DATA/$SBID1/inverted_linefinder.tar.gz flash@$CLIENT:$PARENTDIR/$SBID1/inverted_outputs/
    scp -i $ORACLE_KEY $DATA/$SBID1/config/* flash@$CLIENT:/mnt/db/data/$SBID1/config/
    scp -i $ORACLE_KEY $DATA/$SBID1/logs/* flash@$CLIENT:/mnt/db/data/$SBID1/logs/
    ...
    ssh -i $ORACLE_KEY flash@$CLIENT "cd ~/src/FLASH/database; python3 db_upload.py -m INVERTED -s $SBID1 -t $TMPDIR -d $PARENTDIR -pw aussrc -cs config -o inverted_outputs -C 'Inverted_linefinder_run'"
