...
BAD_FILES_DIR="$PARENT_DIR/bad_ascii_files"
...
for SBID1 in "${SBIDARRAY[@]}"; do
    ...
    if [ ! -d "$DIR1" ]; then
        ...
        cd $DETECTDIR
        cp slurm_linefinder.ini model.txt $PARENT1/config
        jid3=$(sbatch $FINDER/slurm_run_flashfinder.sh $PARENT1 spectra_ascii $BAD_FILES_DIR $SBID1)
        j3=$(echo $jid3 | awk '{print $4}')
        echo "Sumbitted detection job $j3"
    fi
